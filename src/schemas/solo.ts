...
export const SoloRyukyokuSchema = BaseRyukyokuSchema.extend({
  tenpaiPlayers: z.array(PlayerPositionSchema).default([]).refine(
    (players) => players.length <= 4,
    {
      message: 'テンパイ者は4人以下である必要があります'
    }
  ).refine(
    (players) => validatePlayerPositions(players),
    {
      message: '無効なプレイヤー位置があります'
    }
  ),
...
export type SoloScoreResultResponse = z.infer<typeof SoloScoreResultResponseSchema>
...
export type SoloGameResultResponse = z.infer<typeof SoloGameResultResponseSchema>
