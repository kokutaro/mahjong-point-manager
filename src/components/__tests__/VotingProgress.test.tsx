...
describe('VotingProgress', () => {
  ...
  test('投票集計が正しく表示される', () => {
    ...
    expect(screen.getByText('1票', { selector: '.text-green-600' })).toBeInTheDocument() // 継続
    expect(screen.getByText('2票', { selector: '.text-red-600' })).toBeInTheDocument() // 終了
    expect(screen.getByText('1票', { selector: '.text-yellow-600' })).toBeInTheDocument() // 保留
  })
  ...
  test('全員終了投票時の判定結果が表示される', () => {
    ...
    expect(screen.getByText('✅ 全員がセッション終了に合意しました')).toBeInTheDocument()
  })
