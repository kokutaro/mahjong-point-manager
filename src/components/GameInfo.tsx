    ...
    <div className="bg-white rounded-lg shadow-lg p-3 sm:p-6 mb-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 sm:mb-4">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3 mt-3 sm:mt-0 w-full sm:w-auto justify-end">
          ...
          {/* ゲーム状態 */}
          <div
            className={`inline-flex items-center px-2 py-1 rounded-full text-xs sm:text-sm font-medium ${getGamePhaseColor(
              gameState.gamePhase
            )}`}
          >
            {gamePhaseInfo.text}
      ...
      <div className="mb-3 sm:mb-4">
        <div className="flex justify-between text-xs text-gray-500 mb-1">
          <span>進行状況</span>
          <span>
            {gameState.currentRound}/{gamePhaseInfo.maxRound}局
          </span>
        </div>
        ...
        <div className="mt-3 sm:mt-4 p-2 sm:p-3 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="text-xs text-blue-700">
            <div className="flex justify-between">
              <span>オーラス条件</span>
              <span>
                {gameState.currentRound >= gamePhaseInfo.maxRound
                  ? "オーラス圏内"
                  : "通常進行"}
              </span>
            </div>
            {gameState.players.some((p) => p.points <= 0) && (
              <div className="text-red-600 mt-1">⚠ トビ発生中</div>
            )}
          </div>
        </div>
      )}
