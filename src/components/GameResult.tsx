...
export default function GameResult({
  ...
  const [isWaitingForSessionVotes, setIsWaitingForSessionVotes] =
    useState(false)
  ...
  useEffect(() => {
    ...
    socketInstance.on("connect", () => {
      ...
      if (user && resultData) {
        console.log(
          `🔌 Attempting to join room: ${resultData.roomCode} with playerId: ${user.playerId}`
        )
        ...
        console.log(
          `🔌 join_room event emitted for room ${resultData.roomCode}`
        )
      }
    })
    ...
    socketInstance.on(
      "continue-vote",
      ({ playerId, vote }: { playerId: string; vote: boolean }) => {
    ...
    socketInstance.on(
      ...
      ({ continueVotes }: { continueVotes: number }) => {
        startNextGameCountdown()
  ...
  const handleSessionVote = async (vote: VoteOption) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/game/${resultData.gameId}/vote-session`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleCancelSessionVote = async () => {
    ...
    try {
      // 投票取り消しをサーバーに送信
      const response = await fetch(
        `/api/game/${resultData.gameId}/cancel-vote-session`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        const remainingVotes = Object.keys(sessionVotes).filter(
          (id) => id !== user.playerId
        )
  ...
  useEffect(() => {
    ...
    if (
      result.action !== "wait" &&
      result.details.votedPlayers === result.details.totalPlayers
    ) {
      // タイムアウトをクリア
      if (voteTimeout) {
        clearTimeout(voteTimeout)
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full text-center">
          ...
          <div className="text-xs text-gray-400 mb-4">
            Debug: resultData is {JSON.stringify(resultData)}
          </div>
          <button
            onClick={onBack}
            className="bg-gray-500 text-white py-2 px-4 rounded-md hover:bg-gray-600"
      ...
      <div className="max-w-4xl mx-auto">
            ...
            <div className="text-gray-600">
              ...
              {resultData.sessionId && (
                <div className="text-sm mb-1">
                  セッション:{" "}
                  {resultData.sessionName || `#${resultData.sessionCode}`}
                </div>
              )}
              <div className="text-sm">終了理由: {resultData.endReason}</div>
                ...
                {resultData.results.map((result) => (
                  <tr key={result.playerId} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div
                        className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getRankColor(result.rank)}`}
                      >
                        <span className="mr-2">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <div
                        className={`text-sm font-mono ${result.uma >= 0 ? "text-green-600" : "text-red-600"}`}
                      >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <div
                        className={`text-lg font-bold font-mono ${result.settlement >= 0 ? "text-green-600" : "text-red-600"}`}
                      >
            ...
            {resultData.results.map((result) => (
              <div key={result.playerId} className="border rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div
                    className={`px-2 py-1 rounded text-sm font-medium ${getRankColor(result.rank)}`}
                  >
                    {result.rank}位
                  </div>
                </div>
                ...
                <div className="space-y-2 text-sm">
                    ...
                    <span className="font-mono">
                      {(() => {
                        if (result.rank === 1) {
                          ...
                          // 1位以外の場合は通常計算
                          const diff =
                            result.finalPoints - resultData.basePoints
                          if (diff >= 0) {
                  ...
                  <div className="flex justify-between">
                    ...
                    <span
                      className={`font-mono ${result.uma >= 0 ? "text-green-600" : "text-red-600"}`}
                    >
                      {formatPoints(result.uma)}
                    </span>
                  </div>
                      ...
                      {(() => {
                        ...
                        if (result.rank === 1) {
                          ...
                          const othersTotal = resultData.results
                            ...
                            .reduce((sum, r) => {
                              const diff =
                                r.finalPoints - (resultData.basePoints || 30000)
                              return (
                                sum +
                                (diff >= 0
                                  ? Math.floor(diff / 1000)
                  ...
                  <div className="flex justify-between font-semibold">
                    ...
                    <span
                      className={`font-mono text-lg ${result.settlement >= 0 ? "text-green-600" : "text-red-600"}`}
                    >
                      {formatSettlement(result.settlement)}
                    </span>
                  </div>
        ...
        <div className="text-center">
          <div className="mb-4 flex flex-wrap justify-center gap-3">
            ...
            {!isSoloPlay &&
              user?.playerId === resultData.hostPlayerId &&
              resultData.sessionId &&
              !isWaitingForVotes && (
                <button
          ...
          {!isSoloPlay && (
            <div className="bg-green-50 p-4 rounded-lg">
                    ...
                    voteStartTime
                      ? Math.max(
                          ...
                          Math.floor(
                            (5 * 60 * 1000 -
                              (Date.now() -
                                new Date(voteStartTime).getTime())) /
                              1000
                          )
                        )
                      : undefined
                  ...
                  {resultData.sessionId && (
                    <div className="mb-4">
                      ...
                      <div className="text-xs text-green-600 mt-2 bg-green-100 p-2 rounded">
                        <strong>継続:</strong> セッションを続ける |{" "}
                        <strong>終了:</strong> セッションを終了する |{" "}
                        <strong>保留:</strong> 他の人の判断を待つ
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="mt-3 text-sm text-green-700">
                {resultData.sessionId ? (
                  <p>
                    <strong>セッション継続:</strong>{" "}
                    全員が合意すると自動的に次局を開始
                    <br />
                    <strong>新しいセッション:</strong>{" "}
                    同じメンバーで新しいセッションを作成
                  </p>
                ) : (
