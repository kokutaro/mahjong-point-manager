...
export const GET = withErrorHandler(
  async (
    ...
    if (multiGame) {
      console.log("Found multiplayer game:", {
        id: multiGame.id,
        status: multiGame.status,
        participantsCount: multiGame.participants.length,
      })
      ...
      const gameState = {
        ...
        gamePhase:
          multiGame.status === "PLAYING"
            ? ("playing" as const)
            : multiGame.status === "FINISHED"
              ? ("finished" as const)
              : ("waiting" as const),
      }
    ...
    if (soloGame) {
      console.log("Found solo game:", {
        id: soloGame.id,
        status: soloGame.status,
        playersCount: soloGame.players.length,
      })
  ...
  async (
    ...
    if (!soloGame) {
      ...
      if (multiGame) {
        throw new AppError(
          "INVALID_ACTION",
          "マルチプレイゲームでは直接的なゲーム操作はサポートされていません",
          { gameMode: "MULTIPLAYER" },
          400
        )
      }
    ...
    if (body.action === "finish") {
      // 最終順位と精算を計算
      const sortedPlayers = [...soloGame.players].sort(
...
export const POST = withErrorHandler(
  async (
    ...
    console.log(
      "Processing unified force-end for gameId:",
      gameId,
      "reason:",
      reason
    )
    ...
    if (multiGame) {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        // 最終順位と精算を計算（現在の点数ベース）
        const participants = multiGame.participants.sort(
          (a, b) => b.currentPoints - a.currentPoints
        )
        ...
        for (let i = 0; i < participants.length; i++) {
          ...
          const rank = i + 1
          const settlement =
            participant.currentPoints - basePoints + (uma[i] || 0)
