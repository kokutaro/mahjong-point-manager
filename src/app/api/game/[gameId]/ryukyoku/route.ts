  ...
  async (
    ...
    if (multiGame) {
      ...
      const multiData: MultiplayerRyukyokuData = {
        ...
        tenpaiPlayers: (validatedData.tenpaiPlayers || []).map((id) =>
          String(id)
        ),
      }
 ...
 * マルチプレイゲームの流局処理
 */
async function processMultiplayerRyukyoku(
  gameId: string,
  validatedData: MultiplayerRyukyokuData
) {
  ...
  const reason =
    ...
    generateRyukyokuReason(validatedData.type, tenpaiPlayers.length)
 ...
 * ソロプレイゲームの流局処理
 */
async function processSoloRyukyoku(
  gameId: string,
  validatedData: SoloRyukyokuData,
