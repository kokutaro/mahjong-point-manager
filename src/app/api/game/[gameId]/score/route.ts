...
// 統合版スコア計算スキーマ
const unifiedScoreRequestSchema = z
  .object({
  ...
  async (
    ...
    if (soloGame) {
      ...
      const soloData: SoloScoreData = {
        winnerId:
          typeof validatedData.winnerId === "number"
            ? validatedData.winnerId
            : parseInt(String(validatedData.winnerId)),
        han: validatedData.han,
        fu: validatedData.fu,
        isTsumo: validatedData.isTsumo,
        loserId: validatedData.loserId
          ? typeof validatedData.loserId === "number"
            ? validatedData.loserId
            : parseInt(String(validatedData.loserId))
          : undefined,
        honba: validatedData.honba || 0,
        kyotaku: validatedData.kyotaku || 0,
      }
      ...
      // 位置の妥当性チェック
      if (
        isNaN(soloData.winnerId) ||
        soloData.winnerId < 0 ||
        soloData.winnerId > 3
      ) {
        throw new AppError(
          "INVALID_PLAYER_POSITION",
          `無効な勝者位置: ${validatedData.winnerId}`,
          {},
          400
        )
      }
      if (
        soloData.loserId !== undefined &&
        (isNaN(soloData.loserId) ||
          soloData.loserId < 0 ||
          soloData.loserId > 3)
      ) {
        throw new AppError(
          "INVALID_PLAYER_POSITION",
          `無効な敗者位置: ${validatedData.loserId}`,
          {},
          400
        )
      }
 ...
 * マルチプレイゲームの点数計算処理
 */
async function processMultiplayerScore(
  gameId: string,
  validatedData: MultiplayerScoreData
) {
  ...
  if (gameState.gamePhase !== "playing") {
    throw new Error(
      `ゲーム状態が無効です。期待: playing, 現在: ${gameState.gamePhase}`
    )
  }

  const winner = gameState.players.find(
    (p) => p.playerId === validatedData.winnerId
  )
...
async function processSoloScore(gameId: string, validatedData: SoloScoreData) {
  ...
  // 勝者の位置を検証
  const winner = gameState.players.find(
    (p) => p.position === validatedData.winnerId
  )
