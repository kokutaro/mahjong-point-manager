...
describe("POST /api/solo/create", () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks()
    ;(auth.authenticatePlayer as jest.Mock).mockResolvedValue({
      success: true,
  ...
  it("should create a new solo game and players on valid input", async () => {
    ...
    const createdPlayers = mockGameData.players.map((p) => ({
      ...p,
      id: `player-id-${p.position}`,
      soloGameId: "new-game-id",
      score: 25000,
    }))
  ...
  it("should return 400 if player names are not unique", async () => {
    ...
    expect(response.status).toBe(400)
    const responseData = await response.json()
    expect(responseData.error.details[0].message).toBe(
      "プレイヤー名に重複があります"
    )
  })
