...
export default function SessionsPage() {
  ...
  const fetchSessions = async (page: number = 1) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error?.message || "セッション一覧の取得に失敗しました"
        )
      }
                        ...
                        <div className="text-right">
                          <div
                            className={`text-lg font-bold ${
                              participant.totalSettlement >= 0
                                ? "text-green-600"
                                : "text-red-600"
                            }`}
                          >
                            {participant.totalSettlement >= 0 ? "+" : ""}
                            {participant.totalSettlement.toLocaleString()}
                          </div>
                          <div className="text-xs text-gray-500">累計精算</div>
                        </div>
                      </div>
                    ))}
                </div>
          ...
          <div className="bg-white rounded-lg shadow-lg p-4 sm:p-6 mt-6">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
              <div className="text-sm text-gray-600">
                ...
                {Math.min(currentPage * sessionsPerPage, totalSessions)} /{" "}
                {totalSessions}件
              </div>
