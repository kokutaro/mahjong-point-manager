...
export default function SoloGamePage({ params }: SoloGamePageProps) {
  ...
  const fetchGameState = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.error?.message || "ゲーム状態の取得に失敗しました"
        )
      }
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-lg shadow-lg p-8">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600">ゲームを開始する準備ができました</p>
            </div>
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <div className="w-10 h-10 bg-orange-100 rounded-full flex items-center justify-center text-orange-700 font-semibold">
                        {getPlayerWind(
                          player.position as 0 | 1 | 2 | 3,
                          gameState.currentOya as 0 | 1 | 2 | 3
                        )}
                      </div>
                      <div>
...
function PlayerCard({
  ...
  return (
    <div
      className={`bg-white rounded-lg shadow-md p-4 ${isOya ? "ring-2 ring-orange-400" : ""}`}
    >
...
function GameActions({ gameState, onStateUpdate, onError }: GameActionsProps) {
  ...
  const [scoreActionType, setScoreActionType] = useState<"tsumo" | "ron">(
    "tsumo"
  )
