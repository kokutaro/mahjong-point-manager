...
describe('SoloCreatePage', () => {
  ...
  it('should switch to custom preset when uma is changed manually', () => {
    ...
    expect(screen.getByText('カスタム').parentElement).toHaveClass('border-orange-500');
  });
  ...
  it('should call create game API and redirect on success', async () => {
    ...
    await waitFor(() => expect(fetch).toHaveBeenCalledWith('/api/solo/create', expect.any(Object)));
    await waitFor(() => expect(mockPush).toHaveBeenCalledWith('/solo/game/test-game-123'));
  });
  ...
  it('should show an error message if player names are duplicated', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('プレイヤー名が重複しています')).toBeInTheDocument();
    });
