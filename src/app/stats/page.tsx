...
export default function StatsPage() {
  ...
  const exportToCsv = () => {
    ...
    link.setAttribute(
      "download",
      `stats_${stats.playerName}_${new Date().toISOString().split("T")[0]}.csv`
    )
  ...
  const getMonthlyChartData = () => {
    ...
    return months.map((month) => ({
      ...
      winRate:
        stats.monthlyStats[month].games > 0
          ? (stats.monthlyStats[month].wins / stats.monthlyStats[month].games) *
            100
          : 0,
              ...
              <div className="bg-white rounded-lg shadow-lg p-4 sm:p-6">
                ...
                <div
                  className={`text-2xl sm:text-3xl font-bold ${
                    stats.totalSettlement >= 0
                      ? "text-green-600"
                      : "text-red-600"
                  }`}
                >
                  {stats.totalSettlement >= 0 ? "+" : ""}
                  {stats.totalSettlement}
                </div>
              </div>
            </div>
                ...
                {Object.entries(stats.rankDistribution).map(([rank, count]) => {
                  ...
                  return (
                    <div key={rank} className="text-center">
                      <div
                        className={`w-12 h-12 mx-auto rounded-full flex items-center justify-center text-lg font-semibold mb-2 ${getRankColor(Number(rank))}`}
                      >
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                          ...
                          <div>
                            ...
                            <div className="font-semibold text-blue-600">
                              {typeStats.averageRank.toFixed(2)}位
                            </div>
                          </div>
                          <div>
                            ...
                            <div
                              className={`font-semibold ${
                                typeStats.totalSettlement >= 0
                                  ? "text-green-600"
                                  : "text-red-600"
                              }`}
                            >
                              {typeStats.totalSettlement >= 0 ? "+" : ""}
                              {typeStats.totalSettlement}
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  {stats.recentGames.map((game, index) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold ${getRankColor(game.rank)}`}
                        >
