...
export class SoloPointManager {
  ...
  async distributeWinPoints(
    ...
    if (isTsumo) {
      ...
      await this.handleRonDistribution(
        winnerPosition,
        loserPosition!,
        scoreResult
      )
  ...
  private async handleTsumoDistribution(
    ...
    for (const player of players) {
      if (player.position === winnerPosition) {
        ...
        console.log(
          `Player position ${player.position}, currentOya: ${currentOya}, isThisPlayerOya: ${isThisPlayerOya}, payment: ${payment}`
        )
  ...
  async finishGame(reason: string): Promise<void> {
    console.log(
      "🏁 Solo finishGame called with reason:",
      reason,
      "soloGameId:",
      this.soloGameId
    )
    ...
    console.log(
      "🏁 Solo Game status updated to FINISHED, calling calculateFinalResults"
    )
    ...
    console.log(
      "🏁 Solo calculateFinalResults completed, creating game end event"
    )
  ...
  async calculateFinalResults(): Promise<void> {
    console.log(
      "🏁 Solo calculateFinalResults called for soloGameId:",
      this.soloGameId
    )
    const players = await this.getPlayers()
    const game = await prisma.soloGame.findUnique({
  ...
  private calculateSettlement(
    ...
    console.log(
      "🏁 Solo Starting settlement calculation with settings:",
      settings
    )
    ...
    console.log(
      `🏁 Solo First place rounded diff: ${firstPlaceRoundedDiff} (= -${othersRoundedDiffTotal})`
    )
    console.log(`🏁 Solo First place uma: ${firstPlaceUma}`)
    console.log(
      `🏁 Solo First place settlement: ${firstPlaceRoundedDiff} + ${firstPlaceUma} = ${resultsWithDiff[0].settlement}`
    )
  ...
  private checkRoundEnd(
    ...
    console.log(
      `🎯 Solo Game end check: gameType=${gameType}, currentRound=${currentRound}, currentOya=${currentOya}`
    )

    if (gameType === "TONPUU") {
      // 東風戦: 東4局終了条件
      console.log(
        `🎯 Solo TONPUU check: currentRound=${currentRound} > 4?`,
        currentRound > 4
      )
      if (currentRound > 4) {
      ...
      // 半荘戦: 南4局終了条件
      console.log(
        `🎯 Solo HANCHAN check: currentRound=${currentRound} > 8?`,
        currentRound > 8
      )
      if (currentRound > 8) {
    ...
    console.log(
      `🎯 Solo Game continues: gameType=${gameType}, round=${currentRound}`
    )
    return { shouldEnd: false }
  }
  ...
  async forceEndGame(reason: string = "強制終了"): Promise<void> {
    console.log(
      "🏁 Solo forceEndGame called with reason:",
      reason,
      "soloGameId:",
      this.soloGameId
    )
    ...
    console.log(
      "🏁 Solo Game status updated to FINISHED in forceEndGame, calling calculateFinalResults"
    )
