...
async function main() {
  ...
  if (isDryRun) {
    console.log('🔍 ドライランモード: 実際の修正は行わず、問題のあるデータのみを表示します')
  } else {
  ...
  try {
    ...
    for (const session of sessions) {
      ...
      for (const game of session.games) {
        for (const gameParticipant of game.participants) {
          ...
          // 最終結果がある場合のみ統計に含める
          if (gameParticipant.finalRank !== null && gameParticipant.settlement !== null) {
            stats.totalGames += 1
      ...
      for (const [playerId, correctStats] of participantStats.entries()) {
        const currentParticipant = session.participants.find(p => p.playerId === playerId)
        
        if (currentParticipant) {
          const hasDiscrepancy = 
            currentParticipant.totalGames !== correctStats.totalGames ||
            currentParticipant.totalSettlement !== correctStats.totalSettlement ||
          ...
          if (hasDiscrepancy) {
            ...
            console.log(`   ❌ ${currentParticipant.player?.name || playerId}: 修正が必要`)
            console.log(`      ゲーム数: ${currentParticipant.totalGames} → ${correctStats.totalGames}`)
            console.log(`      累計精算: ${currentParticipant.totalSettlement} → ${correctStats.totalSettlement}`)
            ...
            console.log(`   ✅ ${currentParticipant.player?.name || playerId}: 統計は正確`)
          }
        }
      }
    ...
    if (fixes.length > 0) {
      ...
      for (const fix of fixes) {
        console.log(`   - ${fix.sessionName || fix.sessionId}: ${fix.participantsFixed}名修正`)
      }
    } else {
